...
class DmDev(commands.Cog):
    ...
    async def _get_all_admins(self, guild: discord.Guild) -> set[discord.User | discord.Member]:
        """Coleta um conjunto de todos os usuários administradores e super admins únicos no servidor fornecido."""
        admin_users = set()
        ...
        with sqlite3.connect(DB_FILE) as conn:
            ...
            for member in guild.members:
                ...
                if admin_role_ids and not admin_users.__contains__(member):
    ...
    async def dmdev(self, ctx: commands.Context, *, message: str):
        ...
        feedback_msg = await ctx.send("🔍 Coletando os administradores deste servidor...")

        admins_to_dm = await self._get_all_admins(ctx.guild)
